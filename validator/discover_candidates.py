...
GENERIC_OWNERS = {
    ...
    "about", "pricing", "enterprise", "customer-stories", "readme",
    "search", "trending", "explore", "settings", "notifications",
    "pulls", "issues", "codespaces", "orgs", "login", "join", "new",
    "site", "security", "apps",
}
...
GENERIC_REPOS = {
    "followers", "following", "stars", "repositories", "projects",
    "packages", "sponsors", "sponsoring",
}
...
def discover(
    ...
    *,
    ...
    for i, src in enumerate(sources):
        ...
        for owner, repo in repos:
            ...
            bucket = hits.setdefault(
                key, {"sources": set(), "first_source": src}
            )
    ...
    for (owner, repo), info in hits.items():
        ...
        in_kg, matched = already_in_kg(
            owner, repo, sug_id, existing_ids, repo_to_id
        )
        candidates.append(
            {
                "owner": owner,
    ...
    candidates.sort(
        key=lambda c: (-c["source_count"], c["full_name"].lower())
    )
    return candidates, fetch_errors, per_source_counts
...
def write_outputs(
    ...
    lines.append("")
    lines.append(
        f"Generated by `validator/discover_candidates.py` on "
        f"{today.isoformat()}."
    )
    if since is not None:
        lines.append(
...
def _parse_since(value: str) -> dt.date:
    try:
        ...
        raise argparse.ArgumentTypeError(
            f"--since expects YYYY-MM-DD: {exc}"
        ) from exc
...
def main(argv: list[str] | None = None) -> int:
    ...
    today = dt.date.today()
    candidates, fetch_errors, per_source_counts = discover(
        sources, since=args.since
    )
